...
async function main() {
  ...
  let auto_vault_address = await declareAnd_auto_vault(
    auto_comp_vault_address,
    cl_vault_address,
    cl_token_address
  );
}
...
async function declareAndCL_Token() {
  ...
  const cl_token_deployed_tx = await deployContract(
    "CLToken",
    cl_token_tx.class_hash,
    []
  );
  console.log(
    "============== CLToken DEPLOYED=========================",
    cl_token_deployed_tx.contract_address
  );
...
async function declareAndCL_Vault(cl_token_address: any) {
  ...
  const cl_vault_address = await deployContract(
    "CLVault",
    cl_vault_tx.class_hash,
    {
      ekubo_positions_contract:
        "0x02e0af29598b407c8716b17f6d2795eca1b471413fa03fb145a5e33722184067", //ekubo position
      bounds_settings: {
        lower: cairo.uint256(1180100000000000000),
        upper: cairo.uint256(1187680000000000000),
        // lower: 1180100000000000000,
      ...
      pool_key: {
        ...
        fee: 30, //@todo input correct fee
        tick_spacing: 1000000999999999900,
        extension:
          "0x005e470ff654d834983a46b8f29dfa99963d5044b993cb7b9c92243a69dab38f", // note check
      }, // pool_key
      ekubo_positions_nft:
        "0x07b696af58c967c1b14c9dde0ace001720635a660a8e90c565ea459345318b30", //ekubo_positions_nft
      ekubo_core:
        "0x00000005dd3D2F4429AF886cD1a3b08289DBcEa99A294197E9eB43b0e0325b4b", //ekubo_core
      oracle:
        "0x005e470ff654d834983a46b8f29dfa99963d5044b993cb7b9c92243a69dab38f", //oracle
      cl_token: cl_token_address,
    }
  );
...
async function declareAnd_auto_comp_vault() {
  console.log(
    " ############################### Deploying Auto Compound Vault!!! ############################### "
  );

  const auto_comp_classhash =
    "0x022cdeceb0ba23d1c5b0dede12066c03a7084fa838ce61f77b1c343e872031bf";

  const swap_hash =
    "0x56c809dd30d3c06e772f82a91d19e2b8553e538ff7e6699b06c82fc4b796eaf";
  const lend_hash =
    "0x6f9c4e995d20490d46e3e743a052ad5b728bc5b58c4281f058f7242ac247ed9";

  const auto_comp_address = await deployContract(
    "HarvestInvestStrat",
    auto_comp_classhash,
    {
      asset:
        "0x01b5bd713e72fdc5d63ffd83762f81297f6175a5e0a4771cdadbc1dd5fe72cb1", // zETH
  ...
  console.log(
    " *********************** Auto Compound Vault DEPLOYED *********************** ",
    auto_comp_address.contract_address
  );
  return auto_comp_address.contract_address;
}

async function declareAnd_auto_vault(
  auto_comp_address: any,
  cl_vault_address: any,
  cl_token_address: any
) {
  console.log(
    " !!!!!!!!!!!!!!!!!!!!!!!!!!!!! Deploying Auto  Vault!!! !!!!!!!!!!!!!!!!!!!!!!!!!!!!! "
  );

  const auto_vault_tx = await myDeclare("AutoVault");

  const auto_vault_deploy_tx = await deployContract(
    "AutoVault",
    auto_vault_tx.class_hash,
    {
      ...
      _admin:
        "0x64b48806902a367c8598f4f95c305e8c1a1acba5f082d294a43793113115691",
      _rebalancer:
        "0x64b48806902a367c8598f4f95c305e8c1a1acba5f082d294a43793113115691", //rebalancer
  ...
  console.log(
    " !!!!!!!!!!!!!!!!!!!!!!!!!!!!! Auto Vault DEPLOYED !!!!!!!!!!!!!!!!!!!!!!!!!!!!! ",
    auto_vault_deploy_tx.contract_address
  );
